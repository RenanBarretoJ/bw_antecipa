-- BW Antecipa - baseline histórica anterior à migration 003.
-- Fonte estrutural: supabase/schema.sql, validada contra
-- scripts/perf9e/bootstrap/schema-base-candidate.sql.
--
-- A composição abaixo preserva mecanicamente tipos, tabelas, índices, triggers,
-- RLS e policies do schema-base. As únicas alterações intencionais são o
-- hardening de SECURITY DEFINER (search_path fixo e ACL mínimo), documentado
-- junto às funções afetadas. Não contém seed, UUID de negócio, URL ou segredo.
...
-- Hardening mínimo da baseline: os helpers são usados pelas policies da própria
-- baseline e devem nascer com execução restrita, sem depender de migrations futuras.
...
REVOKE ALL ON FUNCTION public.get_user_sacado_cnpj() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_user_operacao_ids() FROM PUBLIC, anon;
...
-- A função de trigger é chamada pelo mecanismo de trigger, não pela Data API.
-- O ACL restrito reproduz o estado seguro observado nos ambientes existentes.
