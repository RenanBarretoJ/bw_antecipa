-- C1.1 hotfix: o fluxo de upload do Consultor ja autorizava criar a NF,
-- instanciar os requisitos e registrar a versao documental. A reconciliacao
-- chamada por essas duas fronteiras, porem, ainda aceitava apenas Gestor e
-- Cedente. Isso fazia o upload ser compensado depois de a NF ter sido criada.
...
BEGIN
  ...
  IF auth.uid() IS NULL OR actor_role NOT IN ('gestor', 'cedente', 'consultor') THEN
  ...
  IF actor_role = 'cedente'
     AND nf_context.cedente_id <> get_user_cedente_id() THEN
...
COMMENT ON FUNCTION public.reconciliar_documentos_base_nf(uuid) IS
  'Reconcilia documentos-base da NF para Gestor, Cedente proprio ou Consultor OWNER/OPERADOR autorizado pela organizacao, carteira e Fundo.';
