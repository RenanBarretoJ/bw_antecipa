...
-- O overlay tem granularidade de NF, mas operacao_calculo_nfs passou a ter
-- granularidade de parcela. A conferencia canonica precisa usar o total das
-- parcelas da mesma operacao/NF/fundo, sem aceitar valores de outro fundo.
...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(coalesce(p_payload->'overlay_itens','[]'::jsonb)) LOOP
    ...
    IF nullif(v_item->>'valor_aquisicao','') IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.operacao_calculo_nfs c
       WHERE c.operacao_id=(v_item->>'operacao_id')::uuid
         AND c.nota_fiscal_id=(v_item->>'nota_fiscal_id')::uuid AND c.fundo_id=v_fundo_id
       GROUP BY c.operacao_id,c.nota_fiscal_id,c.fundo_id
       HAVING count(*)=count(c.valor_presente)
          AND sum(c.valor_presente)=(v_item->>'valor_aquisicao')::numeric
...
REVOKE ALL ON FUNCTION public.persistir_exposicao_execucao(jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.persistir_exposicao_execucao(jsonb) TO service_role;
