...
-- P14: operacoes_nfs preserva o historico da participacao da NF. Somente
-- vinculos com operacoes em status reservante podem impedir um novo submit.
-- A unica alteracao de semantica autorizada neste ticket e que REPROVADA nao
-- reserva mais a NF. CANCELADA continua reservante no submit legado.
...
AS $$
  SELECT p_status IN (
    ...
    'cancelada'::public.operacao_status
  );
$$;
...
COMMENT ON FUNCTION private.operacao_status_reserva_nf(public.operacao_status) IS
  'P14: fonte unica dos status de operacao que reservam NF no submit; reprovada nao reserva e cancelada preserva o baseline legado.';

-- Sobrecarga legada sem p_parcela_ids. Corpo preservado; somente a checagem
-- de vinculo passa a distinguir historico reprovado de reserva ativa.
...
-- Sobrecarga atual com p_parcela_ids. O caminho de parcelas permanece
-- inalterado; a regra P14 alcanca apenas NFs sem parcelas.
CREATE OR REPLACE FUNCTION public.solicitar_operacao_antecipacao_atomica(
...
BEGIN
  ...
  -- NFs sem parcelas: operacoes_nfs e historico. Somente status reservantes
  -- bloqueiam; reprovada e ignorada e cancelada preserva o baseline atual.
  SELECT count(*) INTO already_linked_count
  FROM public.operacoes_nfs onf
  JOIN public.operacoes op ON op.id = onf.operacao_id
  WHERE onf.nota_fiscal_id = ANY(p_nota_fiscal_ids)
  ...
  -- NFs COM parcelas -- a trava e por parcela disponivel e permanece igual.
