-- Corrige a comparação do status enum no trigger de histórico do aceite.
-- COALESCE(NEW.status, '') força o PostgreSQL a converter a string vazia
-- para nf_status antes da comparação, causando erro em qualquer atualização
-- para um status válido que não seja aceite/contestacao.
...
BEGIN
  IF OLD.status IS NOT DISTINCT FROM NEW.status
     OR NEW.status::text NOT IN ('aceita', 'contestada') THEN
    RETURN NEW;
  END IF;

  v_acao := CASE WHEN NEW.status::text = 'aceita' THEN 'aceitar' ELSE 'contestar' END;
  v_tipo_evento := CASE
    WHEN NEW.status::text = 'aceita' THEN 'cessao_aceita_sacado'
    ELSE 'cessao_contestada_sacado'
  END;
  v_categoria := CASE WHEN NEW.status::text = 'aceita' THEN 'aprovacao' ELSE 'reprovacao' END;
  ...
  LOOP
    v_descricao := CASE
      WHEN NEW.status::text = 'aceita' THEN format('Sacado aceitou a cessao da NF %s.', NEW.numero_nf)
