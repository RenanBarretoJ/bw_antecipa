-- Reconciliacao definitiva dos documentos-base da NF com os requisitos da politica.
--
-- XML e DANFE continuam existindo no snapshot da politica. A reconciliacao
-- somente os retira da camada visual quando existe um documento V2 realmente
-- vinculado a mesma NF e ao tipo documental exato.
--
-- A regra de validade vem do snapshot da instancia:
--   estrutural -> evidencia valida pode ser aprovada pelo sistema;
--   manual/hibrido -> evidencia fica pendente ate analise do gestor.

ALTER TABLE public.documento_requisito_instancias
  ADD COLUMN IF NOT EXISTS origem_snapshot text NOT NULL DEFAULT 'upload_requisito';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'public.documento_requisito_instancias'::regclass
      AND conname = 'documento_requisito_origem_snapshot_check'
  ) THEN
    ALTER TABLE public.documento_requisito_instancias
      ADD CONSTRAINT documento_requisito_origem_snapshot_check
      CHECK (origem_snapshot IN ('upload_requisito', 'documento_base_nf', 'reparo_administrativo'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_documento_requisito_nf_origem
  ON public.documento_requisito_instancias(nota_fiscal_id, origem_snapshot)
...
-- O registro do documento atualiza documento_id depois de criar a versao.
-- Este trigger garante que uploads posteriores tambem reconciliem o checklist,
-- sem depender da abertura de uma tela.
...
DROP TRIGGER IF EXISTS documento_requisito_reconciliar_base_nf
  ON public.documento_requisito_instancias;

CREATE TRIGGER documento_requisito_reconciliar_base_nf
  AFTER INSERT OR UPDATE OF documento_id
  ON public.documento_requisito_instancias
  FOR EACH ROW
  EXECUTE FUNCTION public.reconciliar_base_nf_apos_vinculo();

REVOKE ALL ON FUNCTION public.reconciliar_base_nf_apos_vinculo() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reconciliar_base_nf_apos_vinculo() TO authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.reconciliar_documentos_base_nf_existentes() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reconciliar_documentos_base_nf_existentes() TO authenticated;
