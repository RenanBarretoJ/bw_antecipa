...
try {
  ...
  const result = await client.query(isSinqiaFinancial ? `select
    ...
    exists(select 1 from information_schema.columns where table_schema='public' and table_name='rlx_importacoes_financeiras' and column_name='integracao_fundo_versao_id') as linhagem,
    not exists (
    ...
    : `select
    to_regclass('public.rlx_importacoes_financeiras') is not null as importacoes,
    ...
    to_regprocedure('public.iniciar_ciclo_importacao_financeira_rlx(uuid,date,text,uuid)') is not null as lock_ciclo`)
