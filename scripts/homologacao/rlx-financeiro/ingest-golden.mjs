...
async function cleanup(client, fundId) {
  const { data: imports, error } = await client.from('rlx_importacoes_financeiras').select('id,storage_path').eq('fundo_id', fundId).eq('origem', 'GOLDEN_DATASET').eq('provedor', 'rlx_golden')
  if (error) throw error
  const ids = (imports || []).map((item) => item.id)
  if (!ids.length) return 0
  await client.from('rlx_estoque_posicoes').delete().in('importacao_id', ids)
  await client.from('rlx_aquisicao_movimentos').delete().in('importacao_id', ids)
  await client.from('rlx_liquidacao_movimentos').delete().in('importacao_id', ids)
  await client.from('rlx_carteira_snapshots').delete().in('importacao_id', ids)
  await client.from('rlx_importacao_linhas').delete().in('importacao_id', ids)
  await client.from('rlx_importacao_arquivos').delete().in('importacao_id', ids)
  await client.from('rlx_importacoes_financeiras').delete().in('id', ids)
...
async function main() {
  ...
  const worker = resolve(process.cwd(), 'scripts/homologacao/rlx-financeiro/ingest-golden-worker.ts')
