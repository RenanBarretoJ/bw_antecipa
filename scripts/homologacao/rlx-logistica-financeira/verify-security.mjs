...
async function visible(table, fundId) {
  const allowed = new Set(['rlx_posicao_logistica_execucoes', 'rlx_posicao_logistica_resultados'])
...
try {
  ...
  const schema = await db.query(`
    ...
    WHERE c.relnamespace='public'::regnamespace
      AND c.relname=ANY($1)
    ORDER BY c.relname
  `, [['rlx_posicao_logistica_execucoes', 'rlx_posicao_logistica_resultados']])
  ...
  check(await visible('rlx_posicao_logistica_execucoes', dataset.mainFund.id) > 0, 'Gestor principal le execucoes do fundo autorizado')
  check(await visible('rlx_posicao_logistica_resultados', dataset.mainFund.id) > 0, 'Gestor principal le resultados do fundo autorizado')
  check(await visible('rlx_posicao_logistica_execucoes', dataset.adversarialFund.id) === 0, 'Gestor principal nao le outro fundo')
  ...
  await actor(ids.gestor)
  check(await visible('rlx_posicao_logistica_resultados', dataset.adversarialFund.id) > 0, 'Gestor do outro fundo le somente sua posicao')
  check(await visible('rlx_posicao_logistica_resultados', dataset.mainFund.id) === 0, 'Gestor do outro fundo nao cruza para o principal')

  await actor(ids.super_admin)
  check(await visible('rlx_posicao_logistica_execucoes', dataset.mainFund.id) === 0, 'Super Admin puro nao recebe acesso operacional')
  await db.query('RESET ROLE')
  await db.query(`INSERT INTO public.usuario_fundos
    (id,usuario_id,fundo_id,perfil_no_fundo,status,principal)
    VALUES (gen_random_uuid(),$1,$2,'gestor','ativo',false)`, [ids.super_admin, dataset.mainFund.id])
  await actor(ids.super_admin)
  check(await visible('rlx_posicao_logistica_resultados', dataset.mainFund.id) > 0, 'Perfil hibrido le fundo com vinculo gestor ativo')
  check(await visible('rlx_posicao_logistica_resultados', dataset.adversarialFund.id) === 0, 'Perfil hibrido nao amplia acesso para outro fundo')
  ...
  for (const [label, userId] of [
    ...
    await actor(userId)
    check(await visible('rlx_posicao_logistica_execucoes', dataset.mainFund.id) === 0, `${label} nao acessa execucoes P2.4`)
    check(await visible('rlx_posicao_logistica_resultados', dataset.mainFund.id) === 0, `${label} nao acessa resultados P2.4`)
  }

  await actor('00000000-0000-0000-0000-000000000000', 'anon')
  await mustDeny('Anon nao le snapshots P2.4', () => db.query('SELECT count(*) FROM public.rlx_posicao_logistica_execucoes'))
  ...
  await db.query(`INSERT INTO public.usuario_fundos
    ...
    VALUES (gen_random_uuid(),$1,$2,'gestor','ativo',false)`, [ids.gestor, dataset.mainFund.id])
  await actor(ids.gestor)
  await mustDeny('Gestor nao insere execucao diretamente', () => db.query('INSERT INTO public.rlx_posicao_logistica_execucoes DEFAULT VALUES'))
  await mustDeny('Gestor nao insere resultado diretamente', () => db.query('INSERT INTO public.rlx_posicao_logistica_resultados DEFAULT VALUES'))
  await mustDeny('Gestor nao executa RPC interna P2.4', () => db.query(`SELECT public.rlx_persistir_posicao_logistica_execucao('{}'::jsonb)`))
