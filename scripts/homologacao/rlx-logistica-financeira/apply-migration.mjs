...
try {
  const existing = await db.query("select to_regclass('public.rlx_posicao_logistica_execucoes') is not null as applied")
  ...
  const gate = await db.query(`select
    to_regprocedure('public.rlx_persistir_posicao_logistica_execucao(jsonb)') is not null
