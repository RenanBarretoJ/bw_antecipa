...
const action = 'BENCHMARK_P26'
if (!assertMutation(args, action, env.projectRef)) {
  console.log(`Preview seguro. O benchmark executa o pipeline JIT idempotente em homologacao. Para executar: --execute --expected-project-ref ${env.projectRef} --confirm ${mutationConfirmation(action, env.projectRef)}`)
  process.exit(0)
}
...
const child = spawnSync(process.execPath, [resolve('node_modules/tsx/dist/cli.mjs'), resolve('scripts/homologacao/financeiro/risco/benchmark-worker.ts')], {
  cwd: process.cwd(),
  env: { ...process.env, NODE_PATH: [shimRoot, process.env.NODE_PATH].filter(Boolean).join(delimiter) },
  stdio: 'inherit',
})
if (child.error) throw child.error
if (child.status !== 0) throw new Error(`Benchmark P2.6 encerrou com codigo ${child.status ?? 'desconhecido'}.`)
