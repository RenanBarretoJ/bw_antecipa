...
try {
  ...
  `, [[
    'importacoes_financeiras', 'importacao_arquivos', 'importacao_linhas',
    'estoque_posicoes', 'aquisicao_movimentos', 'liquidacao_movimentos',
    'carteira_snapshots', 'importacao_ciclos',
  ]])
  ...
  check(!policies.rows.some((row) => row.qual === 'true' || row.with_check === 'true'), 'nao ha policy financeira USING/WITH CHECK true', policies.rows)
  ...
  `, [[
    'estoque_posicoes', 'aquisicao_movimentos', 'liquidacao_movimentos', 'carteira_snapshots',
  ...
  const numericTypes = await db.query(`
    ...
    WHERE table_schema='public' AND (
      ...
      (table_name='carteira_snapshots' AND column_name='patrimonio_liquido')
    )
  `, [[
    'importacoes_financeiras', 'estoque_posicoes',
    'aquisicao_movimentos', 'liquidacao_movimentos',
    'estoque_atual', 'aquisicoes_atuais', 'liquidacoes_atuais',
  ]])
  ...
  const imports = await db.query(`
    SELECT i.*,
      (SELECT count(*)::integer FROM public.importacao_linhas l WHERE l.importacao_id=i.id) AS staging_rows,
      (SELECT count(*)::integer FROM public.importacao_arquivos a WHERE a.importacao_id=i.id) AS raw_rows
  ...
  const largeId = await db.query(`
    ...
    FROM public.estoque_atual WHERE id_recebivel='900719925474099312345'
  `)
  ...
  const acquisitions = await db.query(`
    SELECT count(*)::integer AS total,count(DISTINCT fingerprint_linha)::integer AS fingerprints,
      bool_and(payload_origem <> '{}'::jsonb) AS payload_ok
    FROM public.aquisicoes_atuais WHERE fundo_id=$1 AND data_referencia IN ('2026-08-07','2026-08-08')
  `, [MAIN_FUND_ID])
  ...
  const liquidations = await db.query(`
    SELECT count(*)::integer AS total,count(DISTINCT fingerprint_linha)::integer AS fingerprints,
      bool_and(payload_origem <> '{}'::jsonb) AS payload_ok
    FROM public.liquidacoes_atuais WHERE fundo_id=$1 AND data_referencia IN ('2026-08-07','2026-08-09')
  `, [MAIN_FUND_ID])
