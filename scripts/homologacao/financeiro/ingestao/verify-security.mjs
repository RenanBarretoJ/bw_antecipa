...
async function count(table, fundId) {
  const allowed = new Set(['estoque_atual', 'importacoes_financeiras', 'importacao_linhas'])
  if (!allowed.has(table)) throw new Error('Tabela de verificacao nao autorizada.')
  const dateFilter = table === 'estoque_atual' ? ' AND data_referencia=$2::date' : ''
  const values = table === 'estoque_atual' ? [fundId, TEST_DATE] : [fundId]
...
try {
  ...
  const currentImport = await db.query(`SELECT id FROM public.importacoes_financeiras WHERE fundo_id=$1 AND tipo_base='ESTOQUE' AND data_referencia=$2 AND status='PUBLICADA' LIMIT 1`, [MAIN_FUND_ID, TEST_DATE])
  check(Boolean(currentImport.rows[0]?.id), 'massa possui importacao vigente para testar publicacao')

  await actor(ids.super_admin)
  check(await count('importacoes_financeiras', MAIN_FUND_ID) > 0, 'Super Admin le importacoes tecnicas')
  check(await count('importacao_linhas', MAIN_FUND_ID) > 0, 'Super Admin le staging tecnico')

  await actor(ids.gestor_a)
  check(await count('estoque_atual', MAIN_FUND_ID) === 89, 'Gestor Fundo A le a base canonica publicada A')
  check(await count('estoque_atual', OTHER_FUND_ID) === 0, 'Gestor Fundo A nao le Fundo B')
  check(await count('importacoes_financeiras', MAIN_FUND_ID) === 0, 'Gestor nao le importacoes tecnicas')
  check(await count('importacao_linhas', MAIN_FUND_ID) === 0, 'Gestor nao le staging tecnico')

  await actor(ids.hibrido)
  check(await count('importacoes_financeiras', OTHER_FUND_ID) > 0, 'Hibrido em contexto Admin administra importacoes A e B')
  check(await count('estoque_atual', MAIN_FUND_ID) === 89, 'Hibrido le canônico do fundo com vinculo gestor')
  check(await count('estoque_atual', OTHER_FUND_ID) === 0, 'Hibrido nao ganha leitura operacional do fundo sem vinculo')
  ...
  for (const [label, id] of [
    ...
    await actor(id)
    check(await count('estoque_atual', MAIN_FUND_ID) === 0, `${label} nao le estoque financeiro global`)
    check(await count('importacoes_financeiras', MAIN_FUND_ID) === 0, `${label} nao le importacao tecnica`)
  }

  await actor('00000000-0000-0000-0000-000000000000', 'anon')
  await mustDeny('Anonimo nao le base canonica', () => db.query('SELECT count(*) FROM public.estoque_atual'))

  await actor(ids.gestor_a)
  await mustDeny('Gestor nao insere diretamente no canonico', () => db.query('INSERT INTO public.estoque_posicoes DEFAULT VALUES'))
  await mustDeny('Gestor nao atualiza diretamente o canonico', () => db.query(`UPDATE public.estoque_posicoes SET provedor=provedor WHERE fundo_id=$1`, [MAIN_FUND_ID]))
  await mustDeny('Gestor nao exclui diretamente do canonico', () => db.query(`DELETE FROM public.estoque_posicoes WHERE fundo_id=$1`, [MAIN_FUND_ID]))
  ...
  await actor(ids.super_admin)
  await mustDeny('Super Admin nao insere diretamente no canonico', () => db.query('INSERT INTO public.estoque_posicoes DEFAULT VALUES'))
  ...
  const firstCycle = await db.query(`SELECT public.iniciar_ciclo_importacao_financeira($1,'2099-12-31','CRON',$2) AS id`, [MAIN_FUND_ID, crypto.randomUUID()])
  const secondCycle = await db.query(`SELECT public.iniciar_ciclo_importacao_financeira($1,'2099-12-31','CRON',$2) AS id`, [MAIN_FUND_ID, crypto.randomUUID()])
