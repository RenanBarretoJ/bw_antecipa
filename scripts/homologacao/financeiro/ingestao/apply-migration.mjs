...
try {
  ...
  const existing = await client.query(isSinqiaFinancialConfig
    ...
    : isSuperAdminHelper
    ? "select to_regprocedure('private.financeiro_usuario_e_super_admin()') is not null as applied"
    : isCapabilities
    ...
    : isHybridScope
    ? "select not exists(select 1 from pg_policies where schemaname='public' and policyname='estoque_super_admin_select') as applied"
    : isRlsHardening
    ? "select exists(select 1 from pg_class where oid='public.estoque_atual'::regclass and reloptions @> array['security_invoker=true']) and to_regclass('public.estoque_chave_nfe_lookup_idx') is not null as applied"
    : isViewRefresh
    ? "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='estoque_atual' and column_name='external_title_key') as applied"
    : isCycleLock
    ? "select to_regprocedure('public.iniciar_ciclo_importacao_financeira(uuid,date,text,uuid)') is not null as applied"
    : isComplement
      ? "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='importacoes_financeiras' and column_name='declaracao_sem_movimento') as applied"
      : "select to_regclass('public.importacoes_financeiras') is not null as applied")
  ...
  const verified = await client.query(isSinqiaFinancialConfig
    ...
    : isSuperAdminHelper
    ? "select to_regprocedure('private.financeiro_usuario_e_super_admin()') is not null as ok"
    : isCapabilities
    ...
    : isHybridScope
    ? "select not exists(select 1 from pg_policies where schemaname='public' and policyname='estoque_super_admin_select') as ok"
    : isRlsHardening
    ? "select exists(select 1 from pg_class where oid='public.estoque_atual'::regclass and reloptions @> array['security_invoker=true']) and to_regclass('public.estoque_chave_nfe_lookup_idx') is not null as ok"
    : isViewRefresh
    ? "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='estoque_atual' and column_name='external_title_key') as ok"
    : isCycleLock
    ? "select to_regprocedure('public.iniciar_ciclo_importacao_financeira(uuid,date,text,uuid)') is not null as ok"
    : isComplement
      ...
      : "select to_regclass('public.importacoes_financeiras') is not null as ok")
