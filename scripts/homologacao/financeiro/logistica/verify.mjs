...
try {
  ...
  const schema = await db.query(`select
    to_regclass('public.posicao_logistica_execucoes') is not null as execucoes,
    to_regclass('public.posicao_logistica_resultados') is not null as resultados,
    to_regprocedure('public.persistir_posicao_logistica_execucao(jsonb)') is not null as rpc,
    (select numeric_scale = 4 from information_schema.columns
      where table_schema='public' and table_name='posicao_logistica_execucoes'
        and column_name='valor_total_aquisicao') as escala_execucao,
    (select numeric_scale = 4 from information_schema.columns
      where table_schema='public' and table_name='posicao_logistica_resultados'
  ...
  const immutableTriggers = await db.query(`select tgname from pg_trigger
    where tgrelid=any($1::regclass[]) and not tgisinternal
      and tgname=any($2)`, [
    ['public.posicao_logistica_execucoes', 'public.posicao_logistica_resultados'],
    ['posicao_logistica_execucoes_imutaveis', 'posicao_logistica_resultados_imutaveis'],
  ])
  ...
  const security = await db.query(`select c.relname,c.relrowsecurity,
    ...
    from pg_class c where c.relnamespace='public'::regnamespace and c.relname=any($1)`, [['posicao_logistica_execucoes', 'posicao_logistica_resultados']])
  ...
  const execution = await db.query(`select * from public.posicao_logistica_execucoes
  ...
  if (execution.rowCount) {
    ...
    const sources = await db.query(`select
      ...
      from public.posicao_logistica_execucoes e
      left join public.importacoes_financeiras i on i.id=e.estoque_importacao_id
      left join public.matching_execucoes m on m.id=e.matching_execucao_id
    ...
    const totals = await db.query(`select count(*)::int total,
      ...
      from public.posicao_logistica_resultados where execucao_id=$1`, [row.id])
    ...
    const lineage = await db.query(`select count(*)::int invalidos from public.posicao_logistica_resultados r
      join public.matching_resultados m on m.id=r.matching_resultado_id
  ...
  const history = await db.query(`select fundo_id,assinatura_execucao,count(*)::int total from public.posicao_logistica_execucoes group by fundo_id,assinatura_execucao having count(*)>1`)
  ...
  const forbidden = await db.query(`select table_name,column_name from information_schema.columns where table_schema='public'
    and table_name=any($1) and column_name in ('status_logistico','logistics_status')`, [['estoque_posicoes','aquisicao_movimentos','liquidacao_movimentos']])
