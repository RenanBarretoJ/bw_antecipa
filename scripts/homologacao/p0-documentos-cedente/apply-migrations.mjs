...
const P0_VERSIONS = new Set(['20260818194455', '20260818195119'])
...
if (apiRef !== EXPECTED_PROJECT_REF || !`${databaseUrl.hostname} ${decodeURIComponent(databaseUrl.username)}`.includes(EXPECTED_PROJECT_REF)) {
...
const unexpectedPending = pendingVersions.filter((version) => !P0_VERSIONS.has(version))
if (unexpectedPending.length > 0) {
  throw new Error(`Aplicacao recusada: existem migrations pendentes fora do P0 (${unexpectedPending.join(', ')}).`)
}
if (pendingVersions.length === 0) {
  console.log(`Migrations do P0 ja constam no historico de homologacao (${apiRef}).`)
  process.exit(0)
}
...
const result = spawnSync(executable, ['migration', 'up', '--db-url', databaseUrl.toString(), '--yes'], {
  cwd: process.cwd(),
  stdio: 'inherit',
  shell: false,
})
...
console.log(`Migrations do P0 aplicadas em homologacao (${apiRef}).`)
