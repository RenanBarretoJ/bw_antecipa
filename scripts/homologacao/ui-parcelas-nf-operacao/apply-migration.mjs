...
const p0Version = '20260820120000'
...
if (unexpectedPending.length > 0) {
  throw new Error(`Aplicacao recusada: existem migrations pendentes fora deste escopo (${unexpectedPending.join(', ')}).`)
}
if (!pendingVersions.includes(p0Version)) {
  console.log(`Migration ja consta no historico de homologacao (${apiRef}).`)
  process.exit(0)
}
...
console.log(`Migration aplicada em homologacao (${apiRef}).`)
