...
const TABLES = [
  'rlx_matching_execucoes',
  'rlx_titulo_nf_vinculos',
  'rlx_titulo_nf_vinculo_chaves',
  'rlx_matching_resultados',
  'rlx_matching_candidatos',
  'rlx_conciliacao_execucoes',
  'rlx_conciliacao_resultados',
]
...
try {
  ...
  check(policies.rows.every((row) => String(row.qual).includes('rlx_gestor_tem_acesso_fundo')), 'leitura exige vinculo gestor ao fundo', policies.rows)
  ...
  const rpcGrants = await db.query(`
    ...
    WHERE specific_schema='public'
      AND routine_name IN ('rlx_persistir_matching_execucao','rlx_persistir_conciliacao_execucao')
  `)
  ...
  const forbidden = await db.query(`
    ...
    WHERE table_schema='public'
      AND table_name IN ('rlx_estoque_posicoes','rlx_aquisicao_movimentos','rlx_liquidacao_movimentos')
  ...
  const executionCounts = await db.query(`
    SELECT
      (SELECT count(*)::integer FROM public.rlx_matching_execucoes) AS matching,
      (SELECT count(*)::integer FROM public.rlx_conciliacao_execucoes) AS conciliacao,
      (SELECT count(*)::integer FROM public.rlx_titulo_nf_vinculos WHERE origem='MANUAL') AS manuais
  `)
  ...
  if (execution.matching > 0) {
    const history = await db.query(`
      ...
      FROM public.rlx_matching_execucoes e
      LEFT JOIN public.rlx_matching_resultados r ON r.execucao_id=e.id
      GROUP BY e.id
    `)
  ...
  if (execution.conciliacao > 0) {
    const history = await db.query(`
      ...
      FROM public.rlx_conciliacao_execucoes
    `)
  ...
  const contradictory = await db.query(`
    ...
    FROM public.rlx_estoque_atual e
  ...
  const d1Acquisitions = await db.query(`
    SELECT id_recebivel FROM public.rlx_aquisicoes_atuais
