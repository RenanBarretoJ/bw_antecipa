...
async function visible(fundId) {
  const result = await db.query('SELECT count(*)::integer AS total FROM public.rlx_matching_resultados WHERE fundo_id=$1', [fundId])
...
try {
  ...
  await mustDeny('Anon nao le resultados P2.3', () => db.query('SELECT count(*) FROM public.rlx_matching_resultados'))
  ...
  `, [gestorA, MAIN_FUND_ID])
  await actor(gestorA)
  await mustDeny('Gestor nao insere resultado diretamente', () => db.query('INSERT INTO public.rlx_matching_resultados DEFAULT VALUES'))
  await mustDeny('Gestor nao executa RPC interna de matching', () => db.query(`SELECT public.rlx_persistir_matching_execucao('{}'::jsonb)`))
  await mustDeny('Match manual cross-fund e bloqueado antes da mutacao', () => db.query(
    'SELECT public.rlx_confirmar_match_manual($1,$2,$3,$4)',
  ...
  await mustDeny('Revogacao manual sem TOTP fresco e bloqueada', () => db.query(
    'SELECT public.rlx_revogar_match_manual($1,$2,$3)', [manualLink, 'Teste sem TOTP', randomUUID()],
  ))
