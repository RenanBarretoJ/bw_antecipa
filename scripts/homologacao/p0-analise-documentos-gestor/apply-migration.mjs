...
const p0Version = '20260819120000'
...
const unexpectedPending = pendingVersions.filter((version) => version !== p0Version)
if (unexpectedPending.length > 0) {
  throw new Error(`Aplicacao recusada: existem migrations pendentes fora deste P0 (${unexpectedPending.join(', ')}).`)
}
if (!pendingVersions.includes(p0Version)) {
  console.log(`Migration P0 ja consta no historico de homologacao (${apiRef}).`)
  process.exit(0)
}
...
if (result.status !== 0) throw new Error(`Falha ao aplicar migration em homologacao (exit ${result.status}).`)
console.log(`Migration P0 aplicada em homologacao (${apiRef}).`)
