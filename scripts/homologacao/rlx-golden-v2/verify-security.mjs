...
const tables = ['rlx_matching_execucoes','rlx_titulo_nf_vinculos','rlx_titulo_nf_vinculo_chaves','rlx_matching_resultados','rlx_matching_candidatos','rlx_conciliacao_execucoes','rlx_conciliacao_resultados']
const failures = []
let checks = 0
...
try {
  ...
  check(policies.rows.every((item) => !item.qual || String(item.qual).includes('rlx_gestor_tem_acesso_fundo')), 'policies permanecem escopadas por fundo', policies.rows)

  const crossFund = await db.query(`SELECT fundo_id,count(*)::int AS total FROM public.rlx_matching_resultados WHERE fundo_id=ANY($1) GROUP BY fundo_id`, [dataset.funds.map((item) => item.id)])
  ...
  const denied = await db.query(`SELECT count(*)::int AS total FROM public.rlx_matching_resultados WHERE fundo_id=ANY($1)`, [dataset.funds.map((item) => item.id)])
