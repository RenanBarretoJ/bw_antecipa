...
import { AUTH_FLOW_COOKIE, getAuthFlowRedirect, isConsultorInviteAllowedPath, isGestorInviteAllowedPath, isMfaSetupAllowedPath, isPasswordRecoveryAllowedPath, lerAuthFlowCookieAssinado } from '@/lib/auth/auth-flow'
...
export async function updateSession(request: NextRequest) {
  ...
  const publicRoutes = ['/', '/login', '/cadastro', '/esqueci-senha', '/redefinir-senha', '/auth/confirm', '/auth/convite-gestor/confirm', '/auth/convite-consultor/confirm', '/convite/cedente', '/convite/gestor', '/convite/consultor']
  ...
  if (authFlow) {
    const allowed = authFlow === 'password_recovery'
      ...
      : authFlow === 'gestor_invite'
        ...
        : authFlow === 'consultor_invite'
          ? isConsultorInviteAllowedPath(pathname)
      : isMfaSetupAllowedPath(pathname)

    if (!user && pathname !== '/redefinir-senha' && pathname !== '/esqueci-senha' && pathname !== '/convite/gestor' && pathname !== '/auth/convite-gestor/confirm' && pathname !== '/convite/consultor' && pathname !== '/auth/convite-consultor/confirm' && pathname !== '/login') {
