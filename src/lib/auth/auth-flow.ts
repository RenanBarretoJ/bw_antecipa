...
export type AuthFlow = 'password_recovery' | 'gestor_invite' | 'consultor_invite' | 'mfa_setup_required' | 'mfa_recovery_temporary'

const AUTH_FLOW_COOKIE_VERSION = 'v1'

export function isAuthFlow(value: string | null | undefined): value is AuthFlow {
  return value === 'password_recovery' || value === 'gestor_invite' || value === 'consultor_invite' || value === 'mfa_setup_required' || value === 'mfa_recovery_temporary'
}
...
export function isConsultorInviteAllowedPath(pathname: string) {
  return pathname === '/convite/consultor' || pathname.startsWith('/mfa') || pathname === '/login'
}
...
export function getAuthFlowRedirect(flow: AuthFlow): string {
  ...
  if (flow === 'consultor_invite') return '/convite/consultor'
  return '/mfa/setup'
}
