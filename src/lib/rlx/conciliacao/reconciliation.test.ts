...
import { RLX_RECONCILIATION_STATUSES, type RlxReconciliationRow, type RlxReconciliationStatus } from './types'

const FUND = '11111111-1111-4111-8111-111111111111'
const ID = '900719925474099312345'

function row(overrides: Partial<RlxReconciliationRow> = {}): RlxReconciliationRow {
  return {
...
function status(input: {
  ...
  acq?: RlxReconciliationRow[]
  liq?: RlxReconciliationRow[]
...
describe('conciliacao RLX_RECON_V1', () => {
  const classificationCases: Array<[RlxReconciliationStatus, Parameters<typeof status>[0]]> = [
  ...
  it('representa alertas documentados somente quando existe evidencia explicita', () => {
    const cases: Array<[RlxReconciliationStatus, Parameters<typeof status>[0]['contexto']]> = [
  ...
  it('bloqueia conciliacao normal quando uma base esta ausente ou incompleta', () => {
    ...
    expect(RLX_RECONCILIATION_STATUSES).toContain('BASE_INCOMPLETA')
  })
})
