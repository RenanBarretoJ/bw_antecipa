...
import {
  RLX_MATCH_RULE_VERSION,
  RLX_RECON_RULE_VERSION,
  type RlxExternalSource,
  type RlxKnownCrosswalk,
  type RlxNoteCandidate,
  type RlxReconciliationRow,
} from './types'
...
function externalSource(row: CanonicalRow, origin: RlxExternalSource['origem']): RlxExternalSource {
...
async function notesForFund(fundoId: string): Promise<RlxNoteCandidate[]> {
  const { data, error } = await admin()
    .from('notas_fiscais')
...
async function crosswalkForFund(fundoId: string): Promise<RlxKnownCrosswalk[]> {
  const { data: links, error: linksError } = await admin()
    .from('rlx_titulo_nf_vinculos')
  ...
  const { data: keys, error: keysError } = await admin()
    .from('rlx_titulo_nf_vinculo_chaves')
  ...
  return (keys || []).map((key: Record<string, unknown>) => {
    ...
    return {
      ...
      notaFiscalId: String(link.nota_fiscal_id), origem: String(link.origem) as RlxKnownCrosswalk['origem'],
      tipoChave: String(key.tipo_chave) as RlxKnownCrosswalk['tipoChave'], valorNormalizado: String(key.valor_normalizado),
    }
  })
}
...
export async function executarMatchingFinanceiro(input: { fundoId: string; dataReferencia: string; atorUsuarioId: string }) {
  ...
  const signature = sha256({ fundoId: input.fundoId, date: input.dataReferencia, inputIds, rule: RLX_MATCH_RULE_VERSION })
  const payload = {
    fundo_id: input.fundoId, data_referencia: input.dataReferencia, regra_versao: RLX_MATCH_RULE_VERSION,
  ...
  const { data, error } = await admin().rpc('rlx_persistir_matching_execucao', { p_payload: payload })
...
function reconRow(row: CanonicalRow, movement: 'ESTOQUE' | 'AQUISICAO' | 'LIQUIDACAO', links: Map<string, RlxKnownCrosswalk>): RlxReconciliationRow {
...
export async function executarConciliacaoFinanceira(input: { fundoId: string; dataReferencia: string; atorUsuarioId: string }) {
  ...
  const matching = await admin().from('rlx_matching_execucoes').select('id')
  ...
  const correlationId = randomUUID()
  const signature = sha256({ fundoId: input.fundoId, date: input.dataReferencia, inputIds, matching: matching.data?.id || null, rule: RLX_RECON_RULE_VERSION })

  if (missing.length > 0) {
    const payload = {
      fundo_id: input.fundoId, data_referencia: input.dataReferencia, regra_versao: RLX_RECON_RULE_VERSION,
    ...
    const { data, error } = await admin().rpc('rlx_persistir_conciliacao_execucao', { p_payload: payload })
