...
import type { RlxLogisticsSnapshotRow } from './types'
...
export function projetarPosicaoLogistica(input: {
  ...
  return {
    ...
    matchingStatus: String(matching?.status || 'NAO_CONCILIADO') as RlxLogisticsSnapshotRow['matchingStatus'],
...
export function somarValoresConhecidos(rows: RlxLogisticsSnapshotRow[]) {
  const sum = (predicate: (row: RlxLogisticsSnapshotRow) => boolean) => {
