...
import { RLX_LOGISTICS_RULE_VERSION } from './types'
...
export async function executarPosicaoLogisticaFinanceira(input: {
  ...
  const client = admin()
  const stockResult = await client.from('rlx_importacoes_financeiras')
  ...
  const matchingResult = await client.from('rlx_matching_execucoes').select('*')
  ...
  const [positionsResult, matchesResult] = await Promise.all([
    client.from('rlx_estoque_posicoes').select('*').eq('fundo_id', input.fundoId).eq('importacao_id', stock.id).order('id'),
    client.from('rlx_matching_resultados').select('*').eq('fundo_id', input.fundoId)
  ...
  const signature = criarAssinaturaPosicaoLogistica({
    ...
    regraVersao: RLX_LOGISTICS_RULE_VERSION,
  })
  ...
  const payload = {
    ...
    regra_versao: RLX_LOGISTICS_RULE_VERSION,
  ...
  const persisted = await client.rpc('rlx_persistir_posicao_logistica_execucao', { p_payload: payload })
  if (persisted.error) throw new Error(`Nao foi possivel persistir a posicao logistica: ${persisted.error.message}`)
  const persistedExecution = await client.from('rlx_posicao_logistica_execucoes')
