...
import type { RlxLinhaProcessada, RlxResultadoParse, RlxTipoBase } from './types'
...
export function processarArquivoRlx(input: {
  ...
  const maxParseMs = input.maxParseMs ?? Number(process.env.RLX_MAX_PARSE_MS || 20_000)
  ...
  const maxRows = input.maxRows ?? Number(process.env.RLX_MAX_IMPORT_ROWS || 100_000)
  ...
  const linhas: RlxLinhaProcessada[] = records.map((record, index) => {
