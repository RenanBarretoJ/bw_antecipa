...
export type ConciliacaoTab = 'visao-geral' | 'matching' | 'conciliacao' | 'logistica' | 'exposicao' | 'excecoes'
...
async function availableDates(
  ...
  const [matching, reconciliation, logistics, exposure] = await Promise.all([
...
async function latestExecutions(
  ...
  const [matchingResult, reconciliationResult, logisticsResult, exposureResult] = await Promise.all([matching.maybeSingle(), reconciliation.maybeSingle(), logistics.maybeSingle(), exposure.maybeSingle()])
