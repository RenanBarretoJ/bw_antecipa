...
import { MATCH_METHODS, RECONCILIATION_STATUSES } from './types'
...
describe('contrato arquitetural P2.3', () => {
  ...
  it('declara todos os metodos e status exigidos pelos expected imutaveis', () => {
    ...
    for (const method of methods) expect(MATCH_METHODS).toContain(method)
    const statuses = new Set(expectedReconciliation.cases.map((item: { expected: string }) => item.expected))
    for (const status of statuses) expect(RECONCILIATION_STATUSES).toContain(status)
  })
