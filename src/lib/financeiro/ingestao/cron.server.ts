...
import { ingerirArquivoFinanceiro, publicarImportacaoFinanceira } from './ingestao.server'
import { obterArquivoCapabilityComTimeout, financialCapabilityHandlerRegistry, FinancialProviderTimeoutError, type FinancialCapabilityHandler } from './provider'
...
type CronDependencies = {
  ...
  handlers?: { get(adapterKey: string, capability: typeof FINANCIAL_CAPABILITIES[number]): FinancialCapabilityHandler | null }
  ingest?: typeof ingerirArquivoFinanceiro
  publish?: typeof publicarImportacaoFinanceira
}
...
export async function executarCicloFinanceiro(dataOperacional: string, dependencies: CronDependencies = {}) {
  const supabase = dependencies.supabase || createAdminClient()
  const resolve = dependencies.resolve || resolverIntegracaoPorCapability
  const handlers = dependencies.handlers || financialCapabilityHandlerRegistry
  const ingest = dependencies.ingest || ingerirArquivoFinanceiro
  const publish = dependencies.publish || publicarImportacaoFinanceira
  const ambiente = integrationRuntimeEnvironment()
  const expectativas = resolverExpectativasCicloFinanceiro(dataOperacional)
  ...
  for (const fund of funds || []) {
    ...
    const { data: cycleId, error: cycleError } = await supabase.rpc('iniciar_ciclo_importacao_financeira', {
      p_fundo_id: fundoId,
    ...
    for (const capability of FINANCIAL_CAPABILITIES) {
      ...
      try {
        ...
        falhas += 1
        const code = error instanceof FinancialProviderTimeoutError ? 'TIMEOUT' : error instanceof SinqiaProviderError ? error.code : 'FALHA'
    ...
    const { error: finishError } = await supabase.from('importacao_ciclos').update({
      status,
