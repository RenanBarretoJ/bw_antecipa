...
import type { IngestaoFinanceiraInput, IngestaoFinanceiraResultado } from './types'
...
function validateInput(input: IngestaoFinanceiraInput) {
...
export async function ingerirArquivoFinanceiro(input: IngestaoFinanceiraInput): Promise<IngestaoFinanceiraResultado> {
  validateInput(input)
  const supabase = createAdminClient()
  ...
  try {
    const { error: importError } = await supabase.from('importacoes_financeiras').insert({
      id: importacaoId,
    ...
    const { error: fileError } = await supabase.from('importacao_arquivos').insert({
    ...
    await audit({ tipoEvento: 'IMPORTACAO_FINANCEIRA_RECEBIDA', importacaoId, fundoId: input.fundoId, atorUsuarioId: input.atorUsuarioId, correlationId, dados: { tipo_base: input.tipoBase, data_referencia: input.dataReferencia, hash_conteudo: hash } })
  } catch (error) {
    await supabase.from('importacoes_financeiras').delete().eq('id', importacaoId)
  ...
  try {
    for (let offset = 0; offset < resultado.linhas.length; offset += 500) {
      ...
      const { error } = await supabase.from('importacao_linhas').insert(rows)
    ...
    const { error: updateError } = await supabase.from('importacoes_financeiras').update({
      status,
      completude: resultado.completude,
    ...
    await audit({ tipoEvento: status === 'VALIDA' ? 'IMPORTACAO_FINANCEIRA_VALIDADA' : 'IMPORTACAO_FINANCEIRA_FALHOU', importacaoId, fundoId: input.fundoId, atorUsuarioId: input.atorUsuarioId, correlationId, dados: { tipo_base: input.tipoBase, data_referencia: input.dataReferencia, linhas_invalidas: invalidas } })
    return { importacaoId, status, duplicada: false, resultado }
  } catch (error) {
    await supabase.from('importacoes_financeiras').update({
      status: 'FALHA',
...
export async function publicarImportacaoFinanceira(importacaoId: string, correlationId = randomUUID()) {
  const supabase = createAdminClient()
