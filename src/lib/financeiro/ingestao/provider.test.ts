...
import { createFinancialCapabilityHandlerRegistry, obterArquivoCapabilityComTimeout, FinancialProviderTimeoutError, type FinancialCapabilityHandler } from './provider'
...
describe('handlers financeiros RLX por capability', () => {
  it('resolve cada handler por adapter e capability sem mega-interface', async () => {
    const handler: FinancialCapabilityHandler = { adapterKey: 'qa_adapter', capability: 'ESTOQUE', obterArquivo: vi.fn().mockResolvedValue(arquivo) }
    const registry = createFinancialCapabilityHandlerRegistry([handler])
  ...
  it('interrompe somente a capability lenta', async () => {
    const handler: FinancialCapabilityHandler = { adapterKey: 'qa_lento', capability: 'ESTOQUE', obterArquivo: () => new Promise(() => undefined) }
    await expect(obterArquivoCapabilityComTimeout(handler, { dataOperacional: '2026-08-10', dataReferencia: '2026-08-07', integrationVersion }, 5)).rejects.toBeInstanceOf(FinancialProviderTimeoutError)
  })
