...
export function identidadeExternaDaFonte(source: FinancialExternalSource) {
...
function resultFromCandidates(
  source: FinancialExternalSource,
  method: MatchMethod,
  candidates: MatchCandidate[],
  knownByNote = new Map<string, KnownCrosswalk>(),
): MatchResult | null {
...
function candidatesFromKnown(
  source: FinancialExternalSource,
  crosswalk: KnownCrosswalk[],
  type: KnownCrosswalk['tipoChave'],
...
export function executarMatchDeterministico(
  ...
  if (key) {
    const candidates = scopedNotes
      ...
      .map((note): MatchCandidate => ({ notaFiscalId: note.id, metodo: 'CHAVE_NFE', evidencias: { chaveNfe: key } }))
...
export function chavesPropagaveis(source: FinancialExternalSource) {
  const values: Array<{ tipo: KnownCrosswalk['tipoChave']; valor: string }> = []
  const add = (
    tipo: KnownCrosswalk['tipoChave'],
