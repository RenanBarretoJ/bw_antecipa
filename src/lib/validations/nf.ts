...
export const notaFiscalSchema = z.object({
  ...
  cnpj_emitente: z.string().min(14, { message: 'CNPJ do emitente invalido.' }).transform((v) => v.replace(/\D/g, '')),
  razao_social_emitente: z.string().min(1, { message: 'Razao social do emitente e obrigatoria.' }),
  cnpj_destinatario: z.string().min(14, { message: 'CNPJ do destinatario invalido.' }).transform((v) => v.replace(/\D/g, '')),
