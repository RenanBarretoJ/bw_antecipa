import { AuthorizationError, requireOperationAccess, requireOperationViewAccess } from '@/lib/auth/authorization'
...
export async function carregarDetalheOperacaoCedente(
  ...
  const access = options.perfil === 'consultor'
    ? await requireOperationViewAccess(operacaoId)
