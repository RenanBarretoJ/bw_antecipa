...
const policy = (): PoliticaResolvida => ({
  ...
  versao: { id: 'version-1', politica_operacional_id: 'policy-1', fundo_id: 'fundo-1', cedente_fundo_id: null, versao: 1, status: 'publicada', vigente_desde: '2026-01-01T00:00:00Z', vigente_ate: null, aceite_sacado_obrigatorio: true, cessao_no_desembolso: true, cria_acompanhamento_entrega: false, exigir_status_logistico_pre_cessao: false, permite_postergacao_upload_canhoto: true, limite_postergacao_upload_canhoto_dias: 5, controle_exposicao_logistica_ativo: false, limite_exposicao_em_transito_pct: null, gate_risco_ativo: false, limite_inclusivo: true, tratamento_pl_indisponivel: 'BLOQUEAR', tratamento_indeterminada: 'REVISAO_MANUAL', tratamento_sem_match: 'BLOQUEAR', tratamento_operacao_nao_incorporada: 'BLOQUEAR', tratamento_liquidacao_parcial: 'SINALIZAR', metodo_calculo_financeiro: 'DIAS_UTEIS_252', tipo_ativo_financeiro: 'NOTA_FISCAL', configuracao: { limite: 100 }, regras: {}, parametros: {}, conteudo_hash: 'draft-hash', publicada_por: 'user-1', publicada_em: '2026-01-01T00:00:00Z', substituida_em: null, created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z' },
  requisitos: [{ id: 'req-1', politica_operacional_versao_id: 'version-1', politica_operacional_id: 'policy-1', fundo_id: 'fundo-1', cedente_fundo_id: null, codigo: 'NF-XML', escopo: 'nf_pre_cessao', tipo_documento_codigo: 'nf_xml', familia_documental: null, documento_tipo_id: null, obrigatorio: true, quantidade_minima: 1, formatos_aceitos: ['xml'], nivel_validacao: 'estrutural', prazo_dias_corridos: 2, momento_obrigatorio: 'nf_pre_cessao', categoria: 'nf_pre_cessao', bloqueia_fluxo: true, observacoes: null, responsavel_upload: 'cedente', responsavel_aprovacao: 'gestor', ordem: 1, ativo: true, created_at: '2026-01-01T00:00:00Z' }],
})
...
describe('snapshot de politica operacional', () => {
  it('serializa de forma deterministica e inclui prazos nos requisitos', () => {
    ...
    expect(first.snapshot.exigir_status_logistico_pre_cessao).toBe(false)
    expect(first.snapshot.calculo_financeiro.metodo).toBe('DIAS_UTEIS_252')
    expect(stableStringify({ b: 2, a: 1 })).toBe('{"a":1,"b":2}')
  })

  it('congela a exigencia logistica pre-cessao no snapshot e no hash', () => {
    const first = criarSnapshotPolitica(policy())
    const changed = policy()
    changed.versao.exigir_status_logistico_pre_cessao = true
    const second = criarSnapshotPolitica(changed)

    expect(second.snapshot.exigir_status_logistico_pre_cessao).toBe(true)
