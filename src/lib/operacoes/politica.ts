...
export async function resolverPoliticaAtiva(
  ...
  const { data: policies, error: policyError } = await context.supabase
    ...
    .select('*')
    .eq('cedente_fundo_id', resolved.cedenteFundo.id)
    .eq('status', 'ativa')
  ...
  const { data: requirements, error: requirementError } = await context.supabase
  ...
  const resolvedPolicy = {
    cedenteFundo: resolved.cedenteFundo,
    fundo: resolved.fundo,
    politica,
    versao,
